...
script_author("alexmarkel0v (јлександр ћаркелов)")
...
function main()
	...
	print('јвтор: јлександр ћаркелов aka alexmarkel0v (vk.com/alexmarkel0v)')
	...
	while true do wait(0)
		if showhud then
			...
			renderDrawBox(ScreenX * (x / ScreenX), (ScreenY * (y / ScreenY)) - 2, ScreenX / 4.5, ScreenY / 250, 0xEEFFFFFF)
			if HP > 0 then
...
function ()
	...
	if mimgui.SliderInt(u8"—мещение по оси Y текста на HUD'e", mimMainTextPos, -16, 16) then
		textposmain = tostring(mimMainTextPos[0])
		settingsIni.main.posmaintext = textposmain
		inicfg.save(mainIni, settings)
	end
	if mimgui.SliderInt(u8"—мещение по оси Y текста выше HUD'a", mimOtherTextPos, -16, 16) then
		textposother = tostring(mimOtherTextPos[0])
		settingsIni.main.posothertext = textposother
		inicfg.save(mainIni, settings)
	end
	if mimgui.SliderInt(u8"—мещение по оси Y прогресс баров", mimProgressPos, -16, 16) then
		progressbarpos = tostring(mimProgressPos[0])
		settingsIni.main.posprogressbar = progressbarpos
		inicfg.save(mainIni, settings)
	end
	if mimgui.SliderInt(u8"—мещение по оси X денег", mimMoneyPos, -18, 18) then
